    ...
    ngx_str_t *responder, ngx_uint_t verify)
{
    ...
    ngx_log_error(NGX_LOG_NOTICE, ssl->log, 0,
                  "using boringssl, currently only \"ssl_stapling_file\" is supported.");
    ...
    if (len <= 0) {
        ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0,
                      "Read ssl_stapling_file file \"%s\" failed: %d", file->data, len);
        return NGX_ERROR;
    }

    if (len >= 1000) {
        ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0,
                      "Unexpected ssl_stapling_file file length: %d", len);
        return NGX_ERROR;
    }
